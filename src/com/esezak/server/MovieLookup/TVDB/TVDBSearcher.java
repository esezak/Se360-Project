...
public class TVDBSearcher {
    ...
    /**
     * @param queryName name of movie/series
     * @return parsed string query for the uri
     */
    ...
    /**
     * @param response that is received from the TVDB
     * @param type content type of the query moveie/series
     * @return the content that is received from TVDB in a usable from
     */
    private static ArrayList<Content> parseJson(String response,ContentType type){
        ...
        for(int i=0; i<jsonArray.length(); i++){
            ...
            if(item.getString("status").equals("released") || item.getString("status").equals("Continuing")){
                release_year = item.getString("year");
                relase_date = item.optString("first_air_time",release_year);
            }else{
                relase_date ="TBA";
            }
            if(ContentType.movie==type){
    ...
    public static void main(String[] args) {
        ArrayList<Content> movieContents = TVDBSearcher.queryFromTVDB("Lord of the rings",ContentType.movie);

        for(Content content : movieContents){
            content.displayContent();
        }
        ArrayList<Content> seriesContents = TVDBSearcher.queryFromTVDB("One Piece",ContentType.series);
        for(Content content : seriesContents){
            content.displayContent();
        }
    }
}
